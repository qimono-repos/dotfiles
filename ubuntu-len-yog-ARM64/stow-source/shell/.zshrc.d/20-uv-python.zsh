...
# --- ARM64: compiled wheels need libz.so.1 from Guix (NOT the host dir) -----
# scipy-openblas/numpy wheels dlopen libz.so.1 at runtime. On this Snapdragon
# host running the Guix python, the system dir /lib/aarch64-linux-gnu MUST NOT
# be added to LD_LIBRARY_PATH — that pulls host glibc-adjacent libs and
# SEGFAULTS/Bus-errors the Guix python. Instead expose the Guix profile's lib
# dir (zlib is a profile package; ~/.guix-profile/lib/libz.so.1 -> store).
if [[ -d "${GUIX_PROFILE:-$HOME/.guix-profile}/lib" ]]; then
  export LD_LIBRARY_PATH="${GUIX_PROFILE:-$HOME/.guix-profile}/lib"
fi
...
_qimono_python_guard() {
  local cmd=$1
  shift
  # Inside a project guix shell, THAT environment's interpreter wins over
  # everything (its manifest may pin a different python than the default
  # profile).
  if [[ -n "${GUIX_ENVIRONMENT:-}" && -x "$GUIX_ENVIRONMENT/bin/$cmd" ]]; then
    "$GUIX_ENVIRONMENT/bin/$cmd" "$@"
    return
  fi
  local resolved
